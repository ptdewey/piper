...
func TestSemanticSpanUsesOnlyBoundedValues(t *testing.T) {
	...
	_, span := StartSpan(context.Background(), SpanName("https://secret.invalid?q=x"), Provider("https://secret.invalid?q=x"), Operation("private operation"))
	...
	for key, want := range map[string]string{
		"piper.provider": "unknown", "piper.operation": "unknown",
...
func TestBoundedAttributeVocabularies(t *testing.T) {
	...
	if got := cacheAttr(CacheOutcome("secret")).Value.AsString(); got != "miss" {
		t.Fatal(got)
	}
}
