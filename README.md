...
Piper emits HTTP server telemetry and semantic signals for provider poll cycles, account processing, MusicBrainz hydration, persistence, and ATProto publication. Telemetry attributes use bounded provider and outcome values; user IDs, account names, DIDs, track metadata, credentials, query strings, and raw errors are not exported.
