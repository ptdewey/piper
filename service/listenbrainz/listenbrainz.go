...
func (s *Service) runPollCycle(ctx context.Context) {
	...
	ctx, span := telemetry.StartSpan(ctx, telemetry.SpanPollCycle, telemetry.ProviderListenBrainz)
	started := time.Now()
	...
	if instruments := telemetry.DefaultInstruments(); instruments != nil {
		instruments.RecordPollCycle(ctx, telemetry.ProviderListenBrainz, outcome, time.Since(started))
	}
}
...
func (s *Service) syncAllUsers(ctx context.Context) (int, error) {
	...
	for _, user := range users {
		...
		accountCtx, span := telemetry.StartSpan(ctx, telemetry.SpanPollAccount, telemetry.ProviderListenBrainz)
		err := s.SyncUser(accountCtx, user)
		outcome, errorType := telemetry.ClassifyError(err)
		telemetry.EndSpan(span, outcome, errorType)
		if instruments := telemetry.DefaultInstruments(); instruments != nil {
			instruments.RecordPollAccount(accountCtx, telemetry.ProviderListenBrainz, outcome)
		}
		if err != nil {
...
func (s *Service) syncListens(ctx context.Context, user *models.User) error {
	...
	for i := range listens {
		...
		if track.RecordingMBID == nil && s.hydrateTrack != nil {
			hydrateCtx, span := telemetry.StartSpan(ctx, telemetry.SpanMusicBrainzHydrate, telemetry.ProviderListenBrainz)
		...
		persistCtx, span := telemetry.StartSpan(ctx, telemetry.SpanDBPersistPlay, telemetry.ProviderListenBrainz)
...
func (s *Service) getJSON(ctx context.Context, path, token string, query url.Values, target any) (err error) {
	ctx, span := telemetry.StartSpan(ctx, telemetry.SpanProviderFetch, telemetry.ProviderListenBrainz)
	defer func() {
		outcome, errorType := telemetry.ClassifyError(err)
		telemetry.EndSpan(span, outcome, errorType)
	}()
